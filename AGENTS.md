...
1.  **Read the task description carefully** (in `docs/burial-register-pilot/TASKS.md`)
2.  **Read relevant part of Technical Design** (`TECH_DESIGN.md`)
3.  **Decide if test-first makes sense** — if yes write tests first; else write code then tests
4.  **Implement changes respecting all rules above**
5.  **Run local checks:**
    ```bash
    npm run lint
    npm test
...
6.  **Run a quick manual functional test** — e.g. process a sample image / page to verify end-to-end behavior
7.  **Ensure no impact on existing memorial workflows**
8.  **Update TASKS.md** to mark the task completed
9.  **Commit changes on a dedicated branch** (e.g. `burial-register-task-xyz`)
